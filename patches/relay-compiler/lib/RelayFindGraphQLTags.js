...
var cache = new (require('./RelayCompilerCache'))('RelayFindGraphQLTags', 'v1');
...
function memoizedFind(tagFinder, text, baseDir, file, options) {
  ...
  return cache.getOrCompute(file.hash + (options.validateNames ? '1' : '0'), find.bind(null, tagFinder, text, require('path').join(baseDir, file.relPath), options));
}
...
function find(tagFinder, text, absPath, _ref) {
  ...
  if (validateNames) {
    var moduleName = require('./getModuleName')(absPath);
...
function validateTemplate(_ref2, moduleName, filePath) {
  ...
  var ast = require('graphql').parse(new (require('graphql').Source)(template, filePath, sourceLocationOffset));
